...
escapeList = np.load('plugins\\esWord.npy',allow_pickle=True).item()
plugin_name ='word'
...
async def words(session: CommandSession):
    ...
    if str(session.ctx['message_type']) == 'private':
        mes_src = 'private'
    else:
        mes_src = str(session.ctx['group_id'])
...
async def delete(session: CommandSession):
    ...
    np.save('plugins\\esWord.npy',escapeList)
